...
<li class="exercise-card">
  ...
  <p class="exercise-card__info">
    ...
    Target: ${r.target}
      ...
      `).join(""))}const F=document.querySelector(".exercises-list"),a=document.querySelector(".modal");F.addEventListener("click",t=>{const e=t.target.closest(".start-btn");if(!e)return;const r=e.dataset.id;C(r)});async function C(t){try{g=t;const e=await fetch(`https://your-energy.b.goit.study/api/exercises/${t}`);if(!e.ok)throw new Error("Failed to fetch exercise");const r=await e.json();P(r),k()}catch(e){console.error(e)}}function P(t){const e=a.querySelector(".modal-gif");e.src=t.gifUrl,e.alt=t.name,a.querySelector(".modal-title").textContent=t.name,a.querySelector(".modal-description").textContent=t.description,a.querySelector(".modal-bodypart").textContent=t.bodyPart,a.querySelector(".modal-equipment").textContent=t.equipment,a.querySelector(".modal-target").textContent=t.target,a.querySelector(".modal-rating").innerHTML=`
  <span class="modal__rating-value">${t.rating.toFixed(1)}</span>
  <div class="modal__stars">
    ${M(t.rating)}
  </div>
`}function M(t){const e=Math.round(t);return Array.from({length:5},(r,n)=>`<span class="modal__star ${n<e?"is-active":""}">★</span>`).join("")}function k(){a.classList.remove("is-hidden"),document.body.style.overflow="hidden"}function j(){a.classList.add("is-hidden"),document.body.style.overflow=""}a.addEventListener("click",t=>{(t.target.classList.contains("modal__backdrop")||t.target.classList.contains("modal__close"))&&j()});async function O(t,e){try{const r=await fetch(`https://your-energy.b.goit.study/api/exercises/${t}/rating`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});if(!r.ok)throw new Error("Failed to send rating");return await r.json()}catch(r){console.error(r)}}const T=document.querySelector(".rating-form");T.addEventListener("submit",async t=>{t.preventDefault();const e=new FormData(t.target),r={rate:Number(e.get("rate")),email:e.get("email"),review:e.get("review")||""},n=await O(g,r);n&&(a.querySelector(".modal-rating").textContent=n.rating,t.target.reset())});const p=document.querySelector(".search-form");p.addEventListener("submit",t=>{t.preventDefault();const r=new FormData(t.target).get("keyword").trim();r&&w(r)});async function w(t,e=1){if(i)try{l=e;const r=new URLSearchParams({keyword:t,page:e,limit:6});c==="Muscles"&&r.append("muscles",i),c==="Body parts"&&r.append("bodypart",i),c==="Equipment"&&r.append("equipment",i);const n=await fetch(`${h}?${r.toString()}`);if(n.status===409){f([]),y(0);return}if(!n.ok)throw new Error("Failed to fetch exercises by keyword");const o=await n.json();d=o.totalPages,f(o.results),y(t)}catch(r){console.error(r)}}function y(t=null){const e=document.querySelector(".pagination-list");if(e){if(d<=1){e.innerHTML="";return}e.innerHTML=Array.from({length:d},(r,n)=>{const o=n+1;return`
        <li>
          <button 
      ...
      `}).join("")}}const A=document.querySelector(".pagination-list");A.addEventListener("click",t=>{var n;const e=t.target.closest(".pagination-btn");if(!e)return;const r=Number(e.dataset.page);(n=p.keyword)!=null&&n.value?w(p.keyword.value,r):b(c,i,r)});
